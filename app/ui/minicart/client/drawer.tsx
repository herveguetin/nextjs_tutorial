...
export default function Drawer({ checkoutData }: { checkoutData: CheckoutData }) {
  ...
  pubSub.on('minicart:drawer:toggle', (event: CustomEventInit) => setIsVisible(event.detail))
    ...
    return (<>
        ...
        <div className="space-y-4 rounded-lg border border-gray-200 bg-white p-4 mt-4 shadow-sm">
          <div className="text-xl font-semibold text-gray-900 ">
            Order summary
            <div>
              <span className="me-2 rounded bg-green-100 px-2.5 py-0.5 text-xs font-medium text-green-800">{loadedData.totals.itemsCount} item(s)</span>
            </div>
          </div>
